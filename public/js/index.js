...
async function registeruser(e) {
  ...
  let response = await fetch("/register", settings); // <--- FIX: store the response
...
async function showAppData(data) {
  if (data.msg) document.getElementById("appData").innerHTML = data.msg;
  else document.getElementById("appData").innerHTML = data;
}
...
window.onload = function () {
  ...
  document.getElementById("loginform").addEventListener("submit", async (e) => {
    loginuser(e).then((data) => saveToken(data));
  });
  ...
  document.getElementById("applink").addEventListener("click", async (e) => {
    getAppData(e).then((data) => showAppData(data));
  });
